...
const EditPostPage = () => {
            ...
            try {
                ...
                if (post) {
                    ...
                    setFormData({
                        ...
                        tags: post.tags || [],
                        coverImagePreview: post.coverImage || undefined,
                        initialCoverImageUrl: post.coverImage || null,
                        coverImage: null,
                        currentTag: "",
                    });
                } else {
    ...
    }, [id, navigate, isAuthenticated, user?.uid]); 
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (formData.coverImage) {
                newCoverImageUrl = await uploadImageToCloudinary(formData.coverImage);
                if (!newCoverImageUrl) {
                    toast.error("Failed to upload new cover image. Please try again.");
                    setIsLoading(false);
                    return;
                }
            }
            else if (!formData.coverImagePreview && formData.initialCoverImageUrl) {
                newCoverImageUrl = null;
            }

            const updateData: Partial<Omit<BlogPost, 'id' | 'authorId' | 'createdAt' | 'updatedAt'>> = {
                title: formData.title,
                categoryId: formData.categoryId,
    ...
    if (postNotFound) {
        toast.error("Something went wrong on our part.");
        navigate("/");
        return null;
    }
        ...
        <div className="w-full relative overflow-hidden min-h-screen">
            ...
            <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8 md:py-16 relative">
                <motion.div
                    ...
                    variants={fadeInUp}
                >
                    <Link to={id ? `/blog/${id}` : '/'} className="flex items-center text-gray-600 dark:text-gray-400 mb-4 hover:text-gray-900 dark:hover:text-white transition-colors">
                    ...
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-gray-900 dark:text-white">
                        Edit <span className="bg-gradient-to-tl from-indigo-600 via-purple-500 to-rose-500 bg-clip-text text-transparent">Blog Post</span>
                    </h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 md:space-y-8">
                    ...
                    <div className="flex justify-between items-center pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
                        <Link to={id ? `/blog/${id}` : '/'} className="px-3 py-2 sm:px-4 sm:py-2 border border-gray-300 dark:border-gray-700 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-500 text-sm">
                            Cancel
                        </Link>
                        <motion.button
